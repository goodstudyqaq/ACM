   ...
   public:
    ...
    Mint work(const string& s) {
        ...
        for (int i = 0; i < sz; i++) {
            int go = s[i] - margin;
            int go2 = go ^ 1;
            if (nodes[now].nxt[go2] == -1) {
            } else {
                int run = nodes[now].nxt[go2];
                int num = nodes[run].cnt;
                if (go == 0) {
                    ans += Mint(one_num + 1) * Mint(num);
                } else {
                    ans += Mint(one_num + 2) * Mint(num);
                }
            }
            now = nodes[now].nxt[go];
            if (go == 1) one_num++;
        }
        int num = nodes[now].cnt;
        ans += Mint(one_num + 1) * Mint(num);
        return ans;
    }
};
...
void solve() {
    ...
    auto work = [&](int idx) -> Mint {
        return trie.work(V[idx]);
    };
