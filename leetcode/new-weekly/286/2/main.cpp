...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        vector<int> queries;
        convert_params(data[i * one_test_number + 0], queries);
        int intLength;
        convert_params(data[i * one_test_number + 1], intLength);
        vector<long long> real_res;
        convert_params(data[i * one_test_number + 2], real_res);

        vector<long long> my_res = sol.kthPalindrome(queries, intLength);
