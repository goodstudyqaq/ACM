...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        string num1;
        convert_params(data[i * one_test_number + 0], num1);
        string num2;
        convert_params(data[i * one_test_number + 1], num2);
        int min_sum;
        convert_params(data[i * one_test_number + 2], min_sum);
        int max_sum;
        convert_params(data[i * one_test_number + 3], max_sum);
        int real_res;
        convert_params(data[i * one_test_number + 4], real_res);

        int my_res = sol.count(num1, num2, min_sum, max_sum);
