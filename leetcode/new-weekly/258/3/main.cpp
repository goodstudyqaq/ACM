...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        vector<int> parents;
        convert_params(data[i * one_test_number + 0], parents);
        vector<int> nums;
        convert_params(data[i * one_test_number + 1], nums);
        vector<int> real_res;
        convert_params(data[i * one_test_number + 2], real_res);

        vector<int> my_res = sol.smallestMissingValueSubtree(parents, nums);
