...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        convert_params(data[i * one_test_number + 0], creators);
        vector<string> ids;
        convert_params(data[i * one_test_number + 1], ids);
        vector<int> views;
        convert_params(data[i * one_test_number + 2], views);
        vector<vector<string>> real_res;
        convert_params(data[i * one_test_number + 3], real_res);

        vector<vector<string>> my_res = sol.mostPopularCreator(creators, ids, views);
