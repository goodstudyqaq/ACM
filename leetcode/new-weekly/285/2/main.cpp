...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        int numArrows;
        convert_params(data[i * one_test_number + 0], numArrows);
        vector<int> aliceArrows;
        convert_params(data[i * one_test_number + 1], aliceArrows);
        vector<int> real_res;
        convert_params(data[i * one_test_number + 2], real_res);

        vector<int> my_res = sol.maximumBobPoints(numArrows, aliceArrows);
