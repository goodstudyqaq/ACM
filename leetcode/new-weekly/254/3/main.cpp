...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        int row;
        convert_params(data[i * one_test_number + 0], row);
        int col;
        convert_params(data[i * one_test_number + 1], col);
        vector<vector<int>> cells;
        convert_params(data[i * one_test_number + 2], cells);
        int real_res;
        convert_params(data[i * one_test_number + 3], real_res);

        int my_res = sol.latestDayToCross(row, col, cells);
