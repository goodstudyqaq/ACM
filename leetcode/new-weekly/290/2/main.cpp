...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        convert_params(data[i * one_test_number + 0], rectangles);
        vector<vector<int>> points;
        convert_params(data[i * one_test_number + 1], points);
        vector<int> real_res;
        convert_params(data[i * one_test_number + 2], real_res);

        vector<int> my_res = sol.countRectangles(rectangles, points);
