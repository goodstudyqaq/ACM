...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        TreeNode* root;
        convert_params(input_param[0], root);
        FindElements sol = FindElements(root);

        for (int j = 1; j < func_num; j++) {
            input_param = split_str_to_params(input_params[j]);
            if (function_names[j] == "find") {
                int target;
                convert_params(input_param[0], target);
                bool real_res;
                convert_params(output_params[j], real_res);
                bool my_res = sol.find(target);
