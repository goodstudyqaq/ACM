...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        int bottom;
        convert_params(data[i * one_test_number + 0], bottom);
        int top;
        convert_params(data[i * one_test_number + 1], top);
        vector<int> special;
        convert_params(data[i * one_test_number + 2], special);
        int real_res;
        convert_params(data[i * one_test_number + 3], real_res);

        int my_res = sol.maxConsecutive(bottom, top, special);
