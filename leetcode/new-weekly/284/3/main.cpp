...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        int src1;
        convert_params(data[i * one_test_number + 2], src1);
        int src2;
        convert_params(data[i * one_test_number + 3], src2);
        int dest;
        convert_params(data[i * one_test_number + 4], dest);
        long long real_res;
        convert_params(data[i * one_test_number + 5], real_res);

        long long my_res = sol.minimumWeight(n, edges, src1, src2, dest);
