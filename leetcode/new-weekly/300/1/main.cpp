...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        int m;
        convert_params(data[i * one_test_number + 0], m);
        int n;
        convert_params(data[i * one_test_number + 1], n);
        ListNode* head;
        convert_params(data[i * one_test_number + 2], head);
        vector<vector<int>> real_res;
        convert_params(data[i * one_test_number + 3], real_res);

        vector<vector<int>> my_res = sol.spiralMatrix(m, n, head);
