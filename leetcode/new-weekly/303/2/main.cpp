...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        vector<string> foods;
        convert_params(input_param[0], foods);
        vector<string> cuisines;
        convert_params(input_param[1], cuisines);
        vector<int> ratings;
        convert_params(input_param[2], ratings);
        FoodRatings sol = FoodRatings(foods, cuisines, ratings);

        for (int j = 1; j < func_num; j++) {
            input_param = split_str_to_params(input_params[j]);
            if (function_names[j] == "changeRating") {
                string food;
                convert_params(input_param[0], food);
                int newRating;
                convert_params(input_param[1], newRating);
                sol.changeRating(food, newRating);
            }
            if (function_names[j] == "highestRated") {
                string cuisine;
                convert_params(input_param[0], cuisine);
                string real_res;
                convert_params(output_params[j], real_res);
                string my_res = sol.highestRated(cuisine);
