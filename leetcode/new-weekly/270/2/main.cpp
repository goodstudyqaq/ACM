...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        TreeNode* root;
        convert_params(data[i * one_test_number + 0], root);
        int startValue;
        convert_params(data[i * one_test_number + 1], startValue);
        int destValue;
        convert_params(data[i * one_test_number + 2], destValue);
        string real_res;
        convert_params(data[i * one_test_number + 3], real_res);

        string my_res = sol.getDirections(root, startValue, destValue);
