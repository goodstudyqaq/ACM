...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        vector<char> keys;
        convert_params(input_param[0], keys);
        vector<string> values;
        convert_params(input_param[1], values);
        vector<string> dictionary;
        convert_params(input_param[2], dictionary);
        Encrypter sol = Encrypter(keys, values, dictionary);

        for (int j = 1; j < func_num; j++) {
            input_param = split_str_to_params(input_params[j]);
            if (function_names[j] == "encrypt") {
                string word1;
                convert_params(input_param[0], word1);
                string real_res;
                convert_params(output_params[j], real_res);
                string my_res = sol.encrypt(word1);
                bool check_result = compare_result(to_string(i + 1) + "-" + to_string(j), my_res, real_res);
                all_test++;
                if (!check_result) fail_test++;
            }
            if (function_names[j] == "decrypt") {
                string word2;
                convert_params(input_param[0], word2);
                int real_res;
                convert_params(output_params[j], real_res);
                int my_res = sol.decrypt(word2);
                bool check_result = compare_result(to_string(i + 1) + "-" + to_string(j), my_res, real_res);
                all_test++;
                if (!check_result) fail_test++;
            }
