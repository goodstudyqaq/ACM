...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        int initialEnergy;
        convert_params(data[i * one_test_number + 0], initialEnergy);
        int initialExperience;
        convert_params(data[i * one_test_number + 1], initialExperience);
        vector<int> energy;
        convert_params(data[i * one_test_number + 2], energy);
        vector<int> experience;
        convert_params(data[i * one_test_number + 3], experience);
        int real_res;
        convert_params(data[i * one_test_number + 4], real_res);

        int my_res = sol.minNumberOfHours(initialEnergy, initialExperience, energy, experience);
