...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        vector<int> arr;
        convert_params(input_param[0], arr);
        RangeFreqQuery sol = RangeFreqQuery(arr);

        for (int j = 1; j < func_num; j++) {
            input_param = split_str_to_params(input_params[j]);
            if (function_names[j] == "query") {
                int left;
                convert_params(input_param[0], left);
                int right;
                convert_params(input_param[1], right);
                int value;
                convert_params(input_param[2], value);
                int real_res;
                convert_params(output_params[j], real_res);
                int my_res = sol.query(left, right, value);
