...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        convert_params(data[i * one_test_number + 0], positions);
        vector<int> healths;
        convert_params(data[i * one_test_number + 1], healths);
        string directions;
        convert_params(data[i * one_test_number + 2], directions);
        vector<int> real_res;
        convert_params(data[i * one_test_number + 3], real_res);

        vector<int> my_res = sol.survivedRobotsHealths(positions, healths, directions);
