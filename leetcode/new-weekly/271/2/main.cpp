...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        vector<int> plants;
        convert_params(data[i * one_test_number + 0], plants);
        int capacityA;
        convert_params(data[i * one_test_number + 1], capacityA);
        int capacityB;
        convert_params(data[i * one_test_number + 2], capacityB);
        int real_res;
        convert_params(data[i * one_test_number + 3], real_res);

        int my_res = sol.minimumRefill(plants, capacityA, capacityB);
