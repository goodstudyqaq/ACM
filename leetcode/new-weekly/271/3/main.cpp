...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        convert_params(data[i * one_test_number + 0], fruits);
        int startPos;
        convert_params(data[i * one_test_number + 1], startPos);
        int k;
        convert_params(data[i * one_test_number + 2], k);
        int real_res;
        convert_params(data[i * one_test_number + 3], real_res);

        int my_res = sol.maxTotalFruits(fruits, startPos, k);
