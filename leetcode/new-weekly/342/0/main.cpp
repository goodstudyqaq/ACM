...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        int arrivalTime;
        convert_params(data[i * one_test_number + 0], arrivalTime);
        int delayedTime;
        convert_params(data[i * one_test_number + 1], delayedTime);
        int real_res;
        convert_params(data[i * one_test_number + 2], real_res);

        int my_res = sol.findDelayedArrivalTime(arrivalTime, delayedTime);
