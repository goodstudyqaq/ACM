...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        int size;
        convert_params(input_param[0], size);
        Bitset sol = Bitset(size);

        for (int j = 1; j < func_num; j++) {
            input_param = split_str_to_params(input_params[j]);
            if (function_names[j] == "fix") {
                int idx;
                convert_params(input_param[0], idx);
                sol.fix(idx);
            }
            if (function_names[j] == "unfix") {
                int idx;
                convert_params(input_param[0], idx);
                sol.unfix(idx);
            }
            if (function_names[j] == "flip") {
                sol.flip();
            }
            if (function_names[j] == "all") {
                bool real_res;
            ...
            if (function_names[j] == "count") {
                int real_res;
                convert_params(output_params[j], real_res);
                int my_res = sol.count();
                bool check_result = compare_result(to_string(i + 1) + "-" + to_string(j), my_res, real_res);
                all_test++;
                if (!check_result) fail_test++;
            }
            if (function_names[j] == "toString") {
                string real_res;
                convert_params(output_params[j], real_res);
                string my_res = sol.toString();
                bool check_result = compare_result(to_string(i + 1) + "-" + to_string(j), my_res, real_res);
                all_test++;
                if (!check_result) fail_test++;
            }
