...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        convert_params(data[i * one_test_number + 0], tires);
        int changeTime;
        convert_params(data[i * one_test_number + 1], changeTime);
        int numLaps;
        convert_params(data[i * one_test_number + 2], numLaps);
        int real_res;
        convert_params(data[i * one_test_number + 3], real_res);

        int my_res = sol.minimumFinishTime(tires, changeTime, numLaps);
