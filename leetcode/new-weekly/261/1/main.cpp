...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        vector<int> rolls;
        convert_params(data[i * one_test_number + 0], rolls);
        int mean;
        convert_params(data[i * one_test_number + 1], mean);
        int n;
        convert_params(data[i * one_test_number + 2], n);
        vector<int> real_res;
        convert_params(data[i * one_test_number + 3], real_res);

        vector<int> my_res = sol.missingRolls(rolls, mean, n);
