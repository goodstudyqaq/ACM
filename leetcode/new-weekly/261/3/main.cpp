...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        int k;
        convert_params(data[i * one_test_number + 1], k);
        char letter;
        convert_params(data[i * one_test_number + 2], letter);
        int repetition;
        convert_params(data[i * one_test_number + 3], repetition);
        string real_res;
        convert_params(data[i * one_test_number + 4], real_res);

        string my_res = sol.smallestSubsequence(s, k, letter, repetition);
