...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        TextEditor sol = TextEditor();

        for (int j = 1; j < func_num; j++) {
            input_param = split_str_to_params(input_params[j]);
            if (function_names[j] == "addText") {
                string text;
                convert_params(input_param[0], text);
                sol.addText(text);
            }
            if (function_names[j] == "deleteText") {
                int k;
                convert_params(input_param[0], k);
                int real_res;
                convert_params(output_params[j], real_res);
                int my_res = sol.deleteText(k);
                bool check_result = compare_result(to_string(i + 1) + "-" + to_string(j), my_res, real_res);
                all_test++;
                if (!check_result) fail_test++;
            }
            if (function_names[j] == "cursorLeft") {
                int k;
                convert_params(input_param[0], k);
                string real_res;
                convert_params(output_params[j], real_res);
                string my_res = sol.cursorLeft(k);
                bool check_result = compare_result(to_string(i + 1) + "-" + to_string(j), my_res, real_res);
                all_test++;
                if (!check_result) fail_test++;
            }
            if (function_names[j] == "cursorRight") {
                int k;
                convert_params(input_param[0], k);
                string real_res;
                convert_params(output_params[j], real_res);
                string my_res = sol.cursorRight(k);
                bool check_result = compare_result(to_string(i + 1) + "-" + to_string(j), my_res, real_res);
                all_test++;
                if (!check_result) fail_test++;
            }
