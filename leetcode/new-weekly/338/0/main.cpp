...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        int numOnes;
        convert_params(data[i * one_test_number + 0], numOnes);
        int numZeros;
        convert_params(data[i * one_test_number + 1], numZeros);
        int numNegOnes;
        convert_params(data[i * one_test_number + 2], numNegOnes);
        int k;
        convert_params(data[i * one_test_number + 3], k);
        int real_res;
        convert_params(data[i * one_test_number + 4], real_res);

        int my_res = sol.kItemsWithMaximumSum(numOnes, numZeros, numNegOnes, k);
