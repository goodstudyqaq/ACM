...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        int k;
        convert_params(data[i * one_test_number + 0], k);
        vector<vector<int>> rowConditions;
        convert_params(data[i * one_test_number + 1], rowConditions);
        vector<vector<int>> colConditions;
        convert_params(data[i * one_test_number + 2], colConditions);
        vector<vector<int>> real_res;
        convert_params(data[i * one_test_number + 3], real_res);

        vector<vector<int>> my_res = sol.buildMatrix(k, rowConditions, colConditions);
