...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        FrequencyTracker sol = FrequencyTracker();

        for (int j = 1; j < func_num; j++) {
            input_param = split_str_to_params(input_params[j]);
            if (function_names[j] == "add") {
                int number;
                convert_params(input_param[0], number);
                sol.add(number);
            }
            if (function_names[j] == "deleteOne") {
                int number;
                convert_params(input_param[0], number);
                sol.deleteOne(number);
            }
            if (function_names[j] == "hasFrequency") {
                int frequency;
                convert_params(input_param[0], frequency);
                bool real_res;
                convert_params(output_params[j], real_res);
                bool my_res = sol.hasFrequency(frequency);
