...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        StockPrice sol = StockPrice();

        for (int j = 1; j < func_num; j++) {
            input_param = split_str_to_params(input_params[j]);
            if (function_names[j] == "update") {
                int timestamp;
                convert_params(input_param[0], timestamp);
                int price;
                convert_params(input_param[1], price);
                sol.update(timestamp, price);
            }
            if (function_names[j] == "current") {
                int real_res;
                convert_params(output_params[j], real_res);
                int my_res = sol.current();
                bool check_result = compare_result(to_string(i + 1) + "-" + to_string(j), my_res, real_res);
                all_test++;
                if (!check_result) fail_test++;
            }
            if (function_names[j] == "maximum") {
                int real_res;
                convert_params(output_params[j], real_res);
                int my_res = sol.maximum();
                bool check_result = compare_result(to_string(i + 1) + "-" + to_string(j), my_res, real_res);
                all_test++;
                if (!check_result) fail_test++;
            }
            if (function_names[j] == "minimum") {
                int real_res;
                convert_params(output_params[j], real_res);
                int my_res = sol.minimum();
                bool check_result = compare_result(to_string(i + 1) + "-" + to_string(j), my_res, real_res);
                all_test++;
                if (!check_result) fail_test++;
            }
