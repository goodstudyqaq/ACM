...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        convert_params(data[i * one_test_number + 1], edges);
        int source;
        convert_params(data[i * one_test_number + 2], source);
        int destination;
        convert_params(data[i * one_test_number + 3], destination);
        int target;
        convert_params(data[i * one_test_number + 4], target);
        vector<vector<int>> real_res;
        convert_params(data[i * one_test_number + 5], real_res);

        vector<vector<int>> my_res = sol.modifiedGraphEdges(n, edges, source, destination, target);
