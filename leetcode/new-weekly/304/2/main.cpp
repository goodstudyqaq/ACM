...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        vector<int> edges;
        convert_params(data[i * one_test_number + 0], edges);
        int node1;
        convert_params(data[i * one_test_number + 1], node1);
        int node2;
        convert_params(data[i * one_test_number + 2], node2);
        int real_res;
        convert_params(data[i * one_test_number + 3], real_res);

        int my_res = sol.closestMeetingNode(edges, node1, node2);
