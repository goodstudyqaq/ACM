...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        vector<int> nums1;
        convert_params(data[i * one_test_number + 0], nums1);
        vector<int> nums2;
        convert_params(data[i * one_test_number + 1], nums2);
        vector<vector<int>> queries;
        convert_params(data[i * one_test_number + 2], queries);
        vector<int> real_res;
        convert_params(data[i * one_test_number + 3], real_res);

        vector<int> my_res = sol.maximumSumQueries(nums1, nums2, queries);
