...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        vector<int> start;
        convert_params(data[i * one_test_number + 0], start);
        vector<int> target;
        convert_params(data[i * one_test_number + 1], target);
        vector<vector<int>> specialRoads;
        convert_params(data[i * one_test_number + 2], specialRoads);
        int real_res;
        convert_params(data[i * one_test_number + 3], real_res);

        int my_res = sol.minimumCost(start, target, specialRoads);
