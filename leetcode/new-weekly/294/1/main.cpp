...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        vector<int> capacity;
        convert_params(data[i * one_test_number + 0], capacity);
        vector<int> rocks;
        convert_params(data[i * one_test_number + 1], rocks);
        int additionalRocks;
        convert_params(data[i * one_test_number + 2], additionalRocks);
        int real_res;
        convert_params(data[i * one_test_number + 3], real_res);

        int my_res = sol.maximumBags(capacity, rocks, additionalRocks);
