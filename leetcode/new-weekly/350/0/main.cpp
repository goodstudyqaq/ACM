...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        int mainTank;
        convert_params(data[i * one_test_number + 0], mainTank);
        int additionalTank;
        convert_params(data[i * one_test_number + 1], additionalTank);
        int real_res;
        convert_params(data[i * one_test_number + 2], real_res);

        int my_res = sol.distanceTraveled(mainTank, additionalTank);
