...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        convert_params(data[i * one_test_number + 1], meetings);
        int firstPerson;
        convert_params(data[i * one_test_number + 2], firstPerson);
        vector<int> real_res;
        convert_params(data[i * one_test_number + 3], real_res);

        vector<int> my_res = sol.findAllPeople(n, meetings, firstPerson);
