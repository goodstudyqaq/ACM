   ...
   public:
    ...
    void init(int _n) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                for (int k = 0; k < n; k++) {
                    if (i && j && k && i != n - 1 && j != n - 1 && k != n - 1) continue;
                    M[{i, j, k}] = idx;
                    idx++;
                }
            }
    ...
    int get_idx(int k, int x, int y, bool out = false) {
        a3 it;
        if (k == 0) {
            it = {x, y, 0};
        } else if (k == 1) {
    ...
    vector<string> reverse(vector<string>& it) {
        int n = it.size();
        vector<string> res(n);
        for (int i = 0; i < n; i++) {
            res[i].resize(n);
    ...
    vector<string> flip(vector<string>& it) {
        int n = it.size();
        vector<string> res(n);
        for (int i = 0; i < n; i++) {
            res[i].resize(n);
    ...
    bool composeCube(vector<vector<string>>& shapes) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            for (int j = 0; j < 4; j++) {
                for (int k = 0; k < 6; k++) {
                    dlx.link(r_idx, i + 1);
                    dlx.link(r_idx + 1, i + 1);
                    for (int x = 0; x < n; x++) {
                        for (int y = 0; y < n; y++) {
                            char c = it[x][y];
                            int idx = get_idx(k, x, y, false);
                            if (c == '1') {
                                dlx.link(r_idx, idx + 6);
                            }

                            c = it2[x][y];
                            if (c == '1') {
                                dlx.link(r_idx + 1, idx + 6);
                            }
                        }
                    }
                    r_idx += 2;
...
int main() {
    ...
    cout << s.composeCube(shapes) << endl;
}