...
void solve() {
    ...
    auto check = [&](int l, int r, int l1, int r1) {
        if (l > l1) {
            swap(l, l1);
            swap(r, r1);
        }
    ...
    auto check2 = [&](int l, int r, int l1, int r1) {
        if (l > l1) {
            swap(l, l1);
            swap(r, r1);
        }
    ...
    auto work = [&](int l, int r) -> int {
        return cnt[uf2.find(color[l])];
    };

    int now = 1;
    while (now <= 2 * n) {
        int go = now;
        int fc = uf.find(color[now]);
        while (go <= 2 * n && uf.find(color[go]) == fc) {
            go++;
        }
        debug(now, go - 1);
        int tmp = work(now, go - 1);
        debug(tmp);
        ans *= Mint(tmp) * 2;
        now = go;
    }
